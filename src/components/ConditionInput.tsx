    ...
    return (
      <FormFields>
        <FormField label="Type">
          <select onChange={e => onChange({ ...value, type: e.target.value as ConditionType })}>
            {
              Object.keys(ConditionType)
                .map(
                  (conditionType) => (
                    <option selected={value.type === conditionType} key={conditionType} value={conditionType}>
        ...
        <FormField label={CONDITION_NAMES[value.type || ConditionType.address]}>
          <input
            type="text"
